...
${createHash('sha256')
  ...
  generateCanonicalQueryString(dateString, xAmzCredential, sessionToken) {
    ...
    canonicalQueryString += `${encodeURIComponent('X-Amz-Algorithm')}=${encodeURIComponent(
      ALGORITHM
    )}&`
    canonicalQueryString += `${encodeURIComponent('X-Amz-Credential')}=${encodeURIComponent(
      xAmzCredential
    )}&`
    canonicalQueryString += `${encodeURIComponent('X-Amz-Date')}=${encodeURIComponent(dateString)}&`
    canonicalQueryString += `${encodeURIComponent('X-Amz-Expires')}=${encodeURIComponent(
      this.ttl
    )}&`

    if (sessionToken)
      canonicalQueryString += `${encodeURIComponent('X-Amz-Security-Token')}=${encodeURIComponent(
        sessionToken
      )}&`

    canonicalQueryString += `${encodeURIComponent('X-Amz-SignedHeaders')}=${encodeURIComponent(
      SIGNED_HEADERS
    )}`
